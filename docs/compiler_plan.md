...
|---|---|---|---|---|---|
| 21 | | **Make the compiler a Scheme library, and split it out of the browser bundle** | — | Moved up by the emitter rewrite (20): the compiled compiler went from 0.44 MB to 1.37 MB -- the emitter is large, the tier's output verbose, and every procedure is emitted twice -- and `dist/scheme.js` from 1.81 to 2.97 MB, all of it paid by every page whether or not it ever compiles anything. A static import in `lowering.js` is why rollup inlines it. The design question is that bootstrapping the compiler is synchronous, and a split means loading its table lazily: the first compile in a page is when it is needed, and the library-load hook in `scheme_entry.js` is the first caller. Both halves change how the compiler is loaded, so they are one task. Today `lowering.js` evaluates a list of files into its own interpreter -- `COMPILER_FILES`, which it has to own because it evaluates them in order and fingerprints them against the prebuilt table -- and the SRFI 1 and SRFI 152 implementations among them are evaluated as top-level code, so their private helpers (`check-procedure`, `cars-of`) become globals of the compiler's environment instead of being imported. As an R7RS library, say `(scheme-js compiler)`, whose `.sld` imports `(srfi 1)` and `(srfi 152)`, includes its files and exports its entry points (`lower-lambda`, `generate-unit`, `inline-expansion-names`, `js-name`, `control-globals`), the file list and order live where Scheme keeps them and `lowering.js` loads the library by name. What has to be worked out: the bootstrap must use the library loader's synchronous path, installing prebuilt code has to replace bindings in library environments (`substituteLibraryValues` already does), and the fingerprint has to cover the library's includes. | R52, R55, R64 |
...
| 30 | ⊘ | **The analyzer** | tier fast enough on its own shape | 2,014 lines, 579 of sets-of-scopes hygiene. **The real self-hosting seam**: until it is ported the AST crosses the boundary on every compile no matter what else is in Scheme. Its own project. Gate on `npm run benchmark:self-host`. | R52 |
| 31 | | **Finish the benchmark suite** | — | Three steps left from the validity review: replace `threads` with the real `threads10` (which also buys vector coverage), add the missing axes (interop, debugger-on, startup), and report a per-stage coverage metric so overfitting stays visible. The review that found the overfitting also listed the fix, and half of it is still undone. | R23 |
| 32 | | **Flonum fast paths** | — | Inline expansions still guard `bigint` only; flonum is 13.1x behind Gambit. They are emitter code: `inline.scm`. | R56 |
| 33 | ⊘ | **Fixnums as JS numbers** | Profiling bignums | R29 ranked this first on the premise that BigInt dominates and code generation cannot help. `fixnum` is now the best tier class, so that premise is false. May still pay — needs a profile, not R29's reasoning. | R29, R39 |
| 34 | | **Hygienic procedural macros, and phase separation** | — | `define-macro` is procedural and non-hygienic, and `core_forms.js:625` still constructs a second `Interpreter` per macro definition — compile time and run time conflated. `syntax-rules` is already hygienic via sets-of-scopes, so the hard part is done. Add explicit renaming (`er-macro-transformer`), keep `define-macro` as a marked legacy extension on top of it, and maintain a phase-1 environment instead of a second interpreter. Blocks nothing today; the expander and compiler must agree on phases eventually. | — |
| 35 | | **Mutable strings, immutable-until-mutated** | — | A real R7RS conformance gap. But `string` is the one class where we beat both references, so it can only cost performance. Use the R31 design and measure. | R31 |
| 36 | | **Drop `source` from runtime `Cons`** | — | Unchanged, unmeasured, low priority. | — |

...
|---|---|---|---|---|---|
| 15 | | **SRFI-125 hash tables** | — | The prerequisite for writing analyses in Scheme. Every analysis needs node-to-information maps and sets, and there are no hash tables at all today; alists are quadratic at the sizes involved — liveness keeps sets of ~500 names across ~2,000 blocks. Build SRFI-125 over a JavaScript `Map` core: the minimal-JavaScript pattern exactly, and `eq?`-keyed tables fall out of `Map`'s object keys. Test first, in both environments. | R55, R56 |
| 16 | ⊘ | **Measure the tier on hash tables and records** | SRFI-125 hash tables | Before anything depends on them. The hottest lesson of the port: `ir.scm` calling interpreted `memq` and `assq` cost 10x, and nobody knew until it was measured. If the tier handles hash-table calls or record access badly, every Scheme analysis runs slow on its hottest path. Record accessors are JavaScript primitives, which should be cheap; confirm it. Gauge with `npm run benchmark:self-host`. | R45 |
...
| 24 | ⊘ | **Move `lift.js` to Scheme** | SRFI-125 hash tables | An IR analysis, so it belongs beside `ir.scm`. Do it as part of whichever task next has to change it — code generation probably will, since direct calls and lifting interact — rather than as a port for its own sake. | R56 |
| 25 | ⊘ | **Move `safety.js` to Scheme** | SRFI-125 hash tables | Consumes exactly what `ir.scm` produces and would share its scope and traversal helpers. It needs environment lookup and closure introspection that Scheme cannot express today, which used to count as a blocker; under the policy it is the minimal JavaScript to expose, not a reason to wait. Like `lift.js`, move it when it is next changed. | R56 |

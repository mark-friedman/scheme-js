...
- **[numeric_tower_discussion.md](numeric_tower_discussion.md)**: analysis and discussion regarding the implementation of the numeric tower, specifically comparing BigInt-based approaches vs. Number-based approaches and their performance implications. Note that [compiler_strategy.md](compiler_strategy.md) measures the total cost of the numeric tower at roughly 3x, against ~200x for interpretive overhead, so the optimizations proposed here are lower-priority than they appear.

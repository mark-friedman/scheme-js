...
| **A compiler tier** | Emits JavaScript for most of the standard library, which is compiled at build time. Per workload class against the interpreter, as the range over two runs: `call` 54–56x, `fixnum` 52–54x, `vector` 22.5–23x, `list` 20–21x, `flonum` 10.5–11x, `continuation` 5x, `bignum` 1.2x, `string` 1.0–1.1x. |
| **A measurement discipline** | 51 vendored canonical benchmarks classified by workload and never blended into one number; cross-implementation comparison against Gambit and Racket; 3,287 tests, including 41 whole programs run under both tiers. |

...
│
│   └── extension/              # Chrome DevTools Extension ("Scheme-JS" panel)
│       ├── manifest.json       # Manifest V3 (debugger permission, content scripts)
│       ├── background.js       # Background service worker (CDP event routing, old probe-based)
...
│       ├── devtools.js         # Creates the top-level "Scheme-JS" DevTools panel
...
│       │   ├── panel.html      # Layout: toolbar, sidebar, editor, call stack, variables
...
│           ├── main.js         # Entry point — init + wire components; calls activate() on
│           │                   # load + page navigation; handles pause/resume messages
│           ├── language/

...
function createSourceList(container, onSelectSource2) {
  ...
  function render() {
    ...
    const items = sources.map((src) => {
      ...
      return `<div class="source-item${isSelected ? " selected" : ""}" data-url="${escapeAttr(src.url)}" title="${escapeAttr(src.url)}">${escapeHtml(name2)}</div>`;
    });
...
async function onBreakpointToggle(line, isNowSet) {
  ...
  if (isNowSet) {
    ...
    if (id2) {
      breakpointIds.set(key, id2);
      saveBreakpointsToPage();
    }
  } else {
    const id2 = breakpointIds.get(key);
    if (id2) {

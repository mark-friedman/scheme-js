...
|---|---|---|---|---|---|
| 18 | | **Tail calls to known loops as JavaScript loops** | — | Every compiled loop pays about 95 ns an iteration to the trampoline: a self tail call compiles to `return new R.TailCall(G(), [args])`, an allocation and a return to the trampoline each time. That is most of why compiled `assq` costs ~300 ns a call on a two-entry list, and why 39% of the compiler's own lowering is spent in `assq` and `memq` -- native versions of just those two make lowering 1.65x faster, which bounds what this is worth there; every compiled program's loops pay the same. Two shapes: a procedure's tail call to itself, and a tail call to a `letrec`-bound local lambda that is only ever called (named `let`, `do`, the `loop` inside `assq`). The analysis that recognises them is Scheme, in or beside `ir.scm`, leaving an annotation the emitter turns into reassigning the parameters and `continue`. A self call through a global must stay guarded, since redefining the global means the call is no longer to itself, and the resumable twin must loop the same way. A/B it per class: R19 is the reminder that a plausible call-path change can be a regression. | R19, R59 |
...
| 24 | | **Split `compiled_compiler.js` out of the browser bundle** | — | 271 KB of `dist/scheme.js` after liveness, needed only by a page that compiles at run time. A static import in `index.js` is why rollup inlines it. Weight rather than urgency. | R52, R55 |

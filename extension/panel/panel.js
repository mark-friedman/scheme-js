...
var ViewState = class {
  ...
  measure() {
    ...
    let refresh2 = this.heightOracle.mustRefreshForWrapping(whiteSpace) || this.mustMeasureContent === "refresh";
...
var StreamLanguage = class _StreamLanguage extends Language {
  constructor(parser4) {
    ...
    let p = fullParser(parser4), self;
    let impl = new class extends Parser {
      createParse(input, fragments, ranges) {
        return new Parse(self, input, fragments, ranges);
      }
    }();
    super(data, impl, [], parser4.name);
    this.topNode = docID(data, this);
    self = this;
    this.streamParser = p;
    this.stateAfter = new NodeProp({ perNode: true });
    this.tokenTable = parser4.tokenTable ? new TokenTable(p.tokenTable) : defaultTokenTable;
  }
  /**
...
function isNumber(token) {
  ...
  return /^[+-]?(\d+\.?\d*|\.\d+)([eE][+-]?\d+)?$/.test(stripped) || /^[+-]?\d+\/\d+$/.test(stripped) || /^[+-]inf\.0$/.test(stripped) || /^[+-]nan\.0$/.test(stripped);
}
var schemeParser = {
  /**
  ...
  token(stream, state) {
    ...
    if (ch === "#") {
      ...
      if (stream.peek() === "t" || stream.peek() === "f" || stream.peek() === "T" || stream.peek() === "F") {
        stream.next();
        return "atom";
      }
...
var schemeLanguage = StreamLanguage.define(schemeParser);
...
var layoutTheme = EditorView.theme({
  "&": {
    height: "100%",
    fontSize: "12px",
...
var BreakpointMarker = class extends GutterMarker {
  ...
  toDOM() {
    ...
    dot2.className = "cm-breakpoint-dot";
    return dot2;
  }
};
...
function createEditor(container, onBreakpointToggle2, onDiamondClick2) {
  ...
  function setContent(content2, language2 = "scheme") {
    let langExtension;
...
async function evalAndParse(expression2, defaultValue, caller) {
  try {
    const json = await evalInPage(expression2);
...
async function getStatus() {
  return evalAndParse(
    ...
    { state: "inactive", reason: null, active: false },
    "getStatus"
  );
}
...
async function removeBreakpoint(id2) {
  return evalAndParse(
    `JSON.stringify(__schemeDebug.removeBreakpoint(${JSON.stringify(id2)}))`,
    false,
    "removeBreakpoint"
  );
}
...
function createSourceList(container, onSelectSource2) {
  ...
  function render() {
    ...
    const items = sources.map((src) => {
      ...
      const name2 = displayName(src.url);
      return `<div class="source-item${isSelected ? " selected" : ""}" data-testid="source-item" data-url="${escapeAttr(src.url)}" title="${escapeAttr(src.url)}">${escapeHtml(name2)}</div>`;
    });
  ...
  async function refresh2() {
    ...
    if (fetched.length > 0) {
      ...
      if (!selectedUrl && sources.length > 0) {
        ...
        render();
        onSelectSource2(sources[0].url, sources[0].content);
      }
    } else {
      render();
    }
  ...
  container.addEventListener("click", (e) => {
    ...
    if (src) {
      selectedUrl = url;
      render();
      onSelectSource2(url, src.content);
    }
  });
...
function createCallStack(container, onSelectFrame2) {
  ...
  function render() {
    ...
    for (let i = frames.length - 1; i >= 0; i--) {
      ...
      const language2 = frame.language || "scheme";
...
// extension/panel-src/protocol/constants.js
...
var PAUSE_CONTEXT = Object.freeze({
...
var attached = false;
var attaching = false;
var scriptUrlMap = /* @__PURE__ */ new Map();
var listeners = {
  paused: [],
  resumed: [],
  scriptParsed: []
...
async function removeJSBreakpoint(breakpointId) {
...
async function setBoundaryBreakpoint() {
  if (!attached) {
    const ok = await attachCDP();
    if (!ok) return null;
    notifyCDPAttached();
...
async function getJSSource(scriptId) {
...
function onCDPPaused(callback) {
  listeners.paused.push(callback);
}
function onCDPResumed(callback) {
  listeners.resumed.push(callback);
}
function onScriptParsed(callback) {
...
function isJSFile(url) {
  ...
  if (lower.endsWith(".js") || lower.endsWith(".mjs") || lower.endsWith(".ts") || lower.endsWith(".jsx") || lower.endsWith(".tsx")) return true;
...
async function setBreakpoint2(url, line, column = null) {
  const lower = url.toLowerCase();
  if (lower.endsWith(".html") || lower.endsWith(".htm")) {
    const isScheme = await isSchemeLine(url, line);
    if (isScheme) {
      return setBreakpoint(url, line, column);
    } else {
      return setJSBreakpoint(url, line - 1);
    }
  }
  if (isJSFile(url)) {
    return setJSBreakpoint(url, line - 1);
...
async function removeBreakpoint2(id2, url) {
  const lower = url.toLowerCase();
  if (lower.endsWith(".html") || lower.endsWith(".htm")) {
    const removedScheme = await removeBreakpoint(id2);
    if (removedScheme) return true;
    return removeJSBreakpoint(id2);
...
function init() {
  onCDPPaused(async (event) => {
    ...
    try {
      const schemeStack = await getStack();
...
async function handleSchemePause(detail) {
  if (detail.reason === "boundary") {
    const bpId = await setBoundaryBreakpoint();
    await resume();
    return;
  }
...
// extension/panel-src/breakpoint-state.js
var breakpointIds = /* @__PURE__ */ new Map();
function bpKey(url, line, column = null) {
  return JSON.stringify([url, line, column]);
}
function parseBpKey(key) {
  const [url, line, column] = JSON.parse(key);
  return { url, line, column };
}
function has(url, line, column = null) {
  return breakpointIds.has(bpKey(url, line, column));
}
function getId(url, line, column = null) {
  return breakpointIds.get(bpKey(url, line, column));
}
function set(url, line, column, id2) {
  breakpointIds.set(bpKey(url, line, column), id2);
}
function remove2(url, line, column = null) {
  breakpointIds.delete(bpKey(url, line, column));
}
function getLinesForUrl(url) {
...
function getAllBreakpoints2() {
  ...
  for (const [key, id2] of breakpointIds.entries()) {
    const bp = parseBpKey(key);
    bps.push({ id: id2, filename: bp.url, line: bp.line, column: bp.column, key });
  }
  return bps;
}
...
async function syncFromInterpreter() {
  try {
...
// extension/panel-src/splitter.js
function initSplitter(splitterEl, { direction, target, min = 50, max = 600, invert = false }) {
  let dragging = false;
  let dragStart = 0;
  let dragStartSize = 0;
...
var breakpointsList = createBreakpointsList(breakpointsContainer, {
  ...
  onRemoveBreakpoint: async (id2, url, line, column) => {
    if (has(url, line, column)) {
      await removeBreakpoint2(id2, url);
      remove2(url, line, column);
...
async function onBreakpointToggle(line, isNowSet) {
  ...
  if (isNowSet) {
    const id2 = await setBreakpoint2(currentSourceUrl, line);
    if (id2) {
      set(currentSourceUrl, line, null, id2);
      persistBreakpointChange();
    }
  } else {
    const id2 = getId(currentSourceUrl, line, null);
    if (id2) {
      await removeBreakpoint2(id2, currentSourceUrl);
      remove2(currentSourceUrl, line, null);
      persistBreakpointChange();
    }
  }
  refreshDiamondMarkers();
  if (isAttached() && !cdpAttached) {
    cdpAttached = true;
    showCDPNotification();
  }
}
async function onDiamondClick(line, column) {
  if (!currentSourceUrl) return;
  const existingId = getId(currentSourceUrl, line, column);
  if (existingId) {
    await removeBreakpoint2(existingId, currentSourceUrl);
    remove2(currentSourceUrl, line, column);
  } else {
    const id2 = await setBreakpoint2(currentSourceUrl, line, column);
    if (id2) {
      set(currentSourceUrl, line, column, id2);
    }
  }
  persistBreakpointChange();
  refreshDiamondMarkers();
...
function refreshDiamondMarkers() {
  ...
  const bpLines = getLinesForUrl(currentSourceUrl);
...
async function loadJSSource(url, scriptId) {
  let content2 = null;
  if (scriptId && isAttached()) {
    content2 = await getJSSource(scriptId);
  }
  if (content2 === null) {
...
async function onPaused2(detail) {
  ...
  if (pauseSource && pauseSource.filename) {
    const isJS = isJSFile(pauseSource.filename);
...
init();
onPaused((event) => onPaused2(event));
onResumed(() => onResumed2());
  ...
  try {
    ...
    if (result.needsReload) {
      ...
      if (status.state === "paused") {
        ...
        onPaused2({ reason: status.reason || "breakpoint", source: status.source || null, stack });
      }
    }
  } catch {
    await refresh();

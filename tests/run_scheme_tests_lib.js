...
export async function runSchemeTests(interpreter, logger, testFiles, fileLoader) {
    ...
    const defineMacroExports = await loadLibrary(['scheme', 'define-macro'], analyze, interpreter, interpreter.globalEnv);
    ...
    applyImports(interpreter.globalEnv, defineMacroExports, { libraryName: ['scheme', 'define-macro'] });

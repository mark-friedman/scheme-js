...
- **[compiler_plan.md](compiler_plan.md)**: *Living.* The only place that ranks compiler work — a single numbered list with status, dependencies, and the finding that justifies each item's position. Completed items keep their numbers and collect at the bottom.
